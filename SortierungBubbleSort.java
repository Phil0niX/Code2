...
class SortierungBubbleSort
{
	...
	public static void insertionSort(int[] array)
	{
		...
		for(int j=1;j<array.length;j++)
		{
			...
			//Initialisierung: i=j-1, A[1..j] \ A[j] ist sortiert
			...
			//Austritt: A[0..length[A]-1] ist sortiert
			array[i+1] = key;
		}
	}
	...
	public static void main(String args[])
	{
		...
		if(args.length >= 2)
		{
			...
			else if(args[1].equals("bubble")) isBubble = true;
		...
		if(args.length == 2)
		{
			if(args[1].equals("insert") || args[1].equals("merge") || args[1].equals("bubble")) auf = true;
			else
			{
				System.out.println("Fehler! Bitte Eingabe ueberpruefen!");
				return;
		...
		else if(isBubble)
		{
			if(auf)
			{
				System.out.println("\nSortierung: BubbleSort aufsteigend!");
				tStart = System.currentTimeMillis();
				// Array mit 500000 absteigenden Werten befuellen
				for(int i=0;i<n;i++)
				{
					array[i] = (n - i);
				}
			...
			{
				System.out.println("\nSortierung: BubbleSort absteigend!");
				tStart = System.currentTimeMillis();
				BubbleSort.bubbleSort(array);
				tEnd = System.currentTimeMillis();
				dreheUm(array);
		...
		msecs = tEnd - tStart;
		System.out.println("Laenge des Arrays: "+array.length);
	...
	public static void merge(int[] array,int unten,int mitte,int oben)
	{
		...
		{
			//assert isSorted(neu[]) : "Array nicht sortiert!";
	...
	// ist Array von Index n bis m sortiert
